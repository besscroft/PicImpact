    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
      {data && data.map((album: AlbumType) => (
        <Card key={album.id} className="flex flex-col h-72 show-up-motion items-center gap-0 py-0">
          ...
          <CardFooter className="flex h-12 p-2 mb-1 space-x-1 select-none rounded-md before:bg-foreground/10 border-foreground/20 border-1 overflow-hidden py-1 before:rounded-xl rounded-large w-[calc(100%_-_8px)] shadow-small z-10">
