...
export default async function List() {
  const getData = async (pageNum: number, Album: string, showStatus: number = -1, camera: string = '', lens: string = '') => {
    'use server'
    return await fetchServerImagesListByAlbum(pageNum, Album, showStatus, camera, lens)
  }

  const getTotal = async (Album: string, showStatus: number = -1, camera: string = '', lens: string = '') => {
    'use server'
